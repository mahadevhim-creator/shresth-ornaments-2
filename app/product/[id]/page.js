...
export default function ProductPage({ params }) {
  ...
  const related = products.filter(p => p.id !== parseInt(params.id)).slice(0, 3)
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-12 pt-[100px] pb-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <div className="relative">
            <div className="aspect-square bg-[#120f08] border border-[#c9a84c]/15 overflow-hidden relative">
              ...
              <div className="absolute top-4 left-4 bg-[#c9a84c] text-[#0a0804] text-[10px] font-black px-3 py-1 tracking-widest uppercase">{product.tag}</div>
            </div>
          ...
          <div className="flex flex-col justify-center">
            ...
            <h1 className="font-serif text-4xl md:text-5xl font-black text-[#f0e6cc] mb-2 leading-tight">{product.name}</h1>
            <div className="w-16 h-0.5 bg-[#c9a84c] mb-6" />
            <p className="text-[#f0e6cc]/60 text-base leading-relaxed mb-8 font-light">{product.desc}</p>
            <div className="bg-[#120f08] border border-[#c9a84c]/15 mb-8">
              {[
                ...
                <div key={i} className={`flex justify-between px-5 py-3 text-sm ${i < 5 ? 'border-b border-[#c9a84c]/10' : ''}`}>
                  ...
                  <span className={`font-bold ${row.label === 'Wastage' ? 'text-[#c9a84c]' : 'text-[#f0e6cc]/80'}`}>{row.value}</span>
                </div>
              ))}
            </div>
            <div className="mb-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Link href="/contact" className="flex-1 bg-[#c9a84c] hover:bg-[#e8c96d] text-[#0a0804] py-4 font-black tracking-widest uppercase text-sm transition text-center">Enquire Now</Link>
              <Link href="/contact" className="flex-1 border border-[#c9a84c]/40 hover:border-[#c9a84c] text-[#c9a84c] py-4 font-bold tracking-widest uppercase text-sm transition text-center">Visit Showroom</Link>
            </div>
            <div className="mt-8 flex flex-wrap gap-3">
              {['BIS Hallmarked', '22KT Certified', 'Authentic Craft', '7-Day Return'].map(b => (
                <span key={b} className="border border-[#c9a84c]/20 text-[#c9a84c]/60 text-[10px] tracking-widest uppercase px-3 py-1">✓ {b}</span>
              ))}
            </div>
          </div>
        </div>
        <div className="mt-24">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {related.map(p => (
              <Link href={`/product/${p.id}`} key={p.id} className="group bg-[#120f08] border border-[#c9a84c]/15 hover:border-[#c9a84c]/50 block overflow-hidden">
                <div className="aspect-square overflow-hidden">
                  <img src={p.img} alt={p.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-85 group-hover:opacity-100" />
                </div>
